...
const TopBarSocials = ({ items, data }) => {
  ...
  return (
    <section
      ref={sectionRef}
      className={`${data?.background_color ? data?.background_color :"bg-primary"} text-white font-paragraph font-bold transition-all duration-300 ${show ? "opacity-100 translate-y-0 relative" : "opacity-0 -translate-y-full hidden"} fixed top-0 left-0 w-full z-50`}
    >
        ...
        <div className="flex gap-4">
          {
            items.map((social, index) => (
              <Tippy
                ...
                <a key={index} className={`text-xl w-8 h-8 flex items-center justify-center  bg-white rounded-full p-2 ${data?.color ? data?.color:"customtext-primary"} `} href={social.url} target="_blank" rel="noopener noreferrer">
